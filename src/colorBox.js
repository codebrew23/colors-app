...
const styles = {
    ...
    copyText: {
        color: props => chroma(props.background).luminance() >= .7 ? 'rgb(0,0,0,0.6)' : 'white'
    },
    colorName: {
    ...
    seeMore: {
        color: props => 
            chroma(props.background).luminance() >= .7 ? 'black' : 'white',
    ...
    copyButton: {
        color: props => 
            chroma(props.background).luminance() >= .7 ? 'rgba(0,0,0,0.6)' : 'white',
        width: '100px',
        height: '30px',
        position: 'absolute',
        display: 'inline-block',
